 ...
 *     at 0x22daec93c. Wire-format struct lives in render_pass.h
 *     under src/protocol/ (kept there because callers from both
 *     handlers/ and any future live-tree consumer can include it
 *     without back-dependencies into handlers/).
 *
 * Ported from pre-refactor src/protocol/render_pass.c at b652199~1
 * (stranded in dead-code-to-revive/ between 2026-05-12 and the
 * 2026-05-13 consolidation pass). Logic unchanged from the
...
int lagfx_parse_render_pass_descriptor(const uint8_t *payload,
    ...
    if (len < LAGFX_RENDER_PASS_WIRE_SIZE) {
        LAGFX_ERR("parse_render_pass: payload too short (%zu < %u)",
