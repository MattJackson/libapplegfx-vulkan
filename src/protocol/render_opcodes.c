 ...
 * guest emits KextCmdExecCmdbuf (opcode 0x20) on a sub-channel.
...
/* Recursive command buffer inner handler — re-enters render decoder
 * with inner command buffer from resource pointer. */
static int lagfx_op_exec_cmdbuf_inner(lagfx_protocol_t *p,
                                               const uint8_t    *payload,
                                               size_t            len) {
    if (!p || !payload || len < 8u) {
        LAGFX_WARN("command buffer inner: payload too short (len=%zu)", len);
        return 0;
    }
    ...
    LAGFX_TRACE("command buffer inner: resource_id=%u pad=0x%08x", 
                resource_id, pad);
    ...
    LAGFX_TRACE("command buffer inner: absorbing (deferred recursion for resource %u)", 
                resource_id);
 ...
 * TSV shows "<default/throw>" suggesting error case or unimplemented cmd. */
    ...
    { 0x3c, "command buffer inner",                     20, 2, lagfx_op_exec_cmdbuf_inner },
