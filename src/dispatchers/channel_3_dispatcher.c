/*
 * libapplegfx-vulkan — Channel 3 dispatcher (Compute/Render)
 * src/dispatchers/channel_3_dispatcher.c
 *
...
lagfx_channel_3_dispatcher_t *channel_3_dispatcher_new(void) {
    lagfx_channel_3_dispatcher_t *d = calloc(1, sizeof(*d));
    if (!d) return NULL;
    
    d->base.name = "Channel3Dispatcher(ch3)";
    d->base.channel_id_min = 3;
    d->base.channel_id_max = 3;
    
    LAGFX_LOG("Channel3Dispatcher: created for channel 3");
    return d;
}

void channel_3_dispatcher_ring_dispatch(lagfx_channel_3_dispatcher_t *d,
                                        struct lagfx_protocol *p,
                                        uint64_t descr_gpa,
                                        uint8_t ch_id) {
    (void)d;
    
    LAGFX_LOG("Channel3Dispatcher(ch%u): processing compute commands", ch_id);
    
    if (!p->dev || !p->dev->desc.shell.read_memory) {
        LAGFX_ERR("Channel3Dispatcher: no device or read_memory callback");
        return;
    }
    ...
    if (!p->dev->desc.shell.read_memory(p->dev->desc.shell.opaque,
        ...
        LAGFX_ERR("Channel3Dispatcher: failed to read descriptor");
        return;
    }
    ...
    LAGFX_LOG("Channel3Dispatcher(ch%u): ring wr=%u rd=%u pfn=0x%x",
              ch_id, write_ptr, read_ptr, ring_pfn);
    
    if (ring_pfn == 0 || write_ptr <= read_ptr) {
        LAGFX_TRACE("Channel3Dispatcher: empty or invalid ring");
        return;
    }
    ...
    while (cur_rp + 12u <= write_ptr && cmd_idx < 256) {
        ...
        for (size_t i = 0; i < 12 && ok; i++) {
            if (!p->dev->desc.shell.read_memory(p->dev->desc.shell.opaque,
                ...
                LAGFX_WARN("Channel3Dispatcher: failed to read header byte %zu", i);
                ok = false;
            }
        }
        ...
        LAGFX_LOG("Channel3Dispatcher(ch%u): opcode=0x%04x length=%u stamp=0x%08x",
        ...
        case 0x37:  /* CmdExecIndirect2/Kext */
            ...
            if (length > sizeof(p->doorbell_bounce_buffer)) {
                LAGFX_ERR("Channel3Dispatcher: cmd_len %u exceeds bounce buffer", length);
                break;
            }
            ...
            for (uint32_t i = 0; i < payload_len && ok; i++) {
                if (!p->dev->desc.shell.read_memory(
                    ...
                    LAGFX_WARN("Channel3Dispatcher: failed to read payload byte %u", i);
                    ok = false;
                }
            }
        ...
        case 0x38:  /* CmdDefineHostTask */
            LAGFX_LOG("Channel3Dispatcher(ch%u): CmdDefineHostTask", ch_id);
            p->extra_stamp_advance = 0;
        ...
        case 0x39:  /* CmdMapMemoryImmediate */
            LAGFX_LOG("Channel3Dispatcher(ch%u): CmdMapMemoryImmediate", ch_id);
            p->extra_stamp_advance = 0;
        ...
        default:
            LAGFX_WARN("Channel3Dispatcher(ch%u): unknown opcode 0x%04x", ch_id, opcode);
            last_stamp = stamp;
            break;
        }
    ...
    LAGFX_LOG("Channel3Dispatcher(ch%u): drained %u cmd(s)", ch_id, cmd_idx);
