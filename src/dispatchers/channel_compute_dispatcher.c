...
size_t channel_compute_dispatcher_drain(lagfx_protocol_t *p, uint32_t chan_id) {
    ...
    LAGFX_LOG("compute drain: chan=%u doorbell=%u", chan_id, write_ptr);
    ...
    for (size_t i = 0; i < max_cmds && cmds_processed < write_ptr; ++i) {
        ...
        if (!p->dev || !((lagfx_device_t *)p->dev)->desc.shell.read_memory) {
            LAGFX_WARN("compute drain: no shell.read_memory callback available");
            break;
        }
        ...
        if (!ok) {
            LAGFX_WARN("compute drain: read_memory failed for GPA 0x%llx", (unsigned long long)cmd_gpa);
            break;
        }
