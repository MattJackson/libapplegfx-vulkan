...
Locates LLVM bitcode magic BC C0 DE in the metallib and treats
everything from that offset onward as a single bitcode module. Pipes
it through llvm-dis, regex-scans for @air.* callsites, accumulates
counts, emits CSV.

For per-function extraction the production pipeline lives at
examples/triangle/extract_retarget.c (build target
`triangle-extract-retarget`) which writes per-function .bc files to
a directory. This script is a research helper; the whole-module
inventory is sufficient for surveying intrinsic vocabulary.

Output CSV columns: intrinsic_name, signature, callsite_count,
first_seen_function. Sorted by callsite_count descending.
"""
import argparse
import csv
import os
...
def main() -> int:
    ...
    scan_ir(ir, function_label="<whole-module>", counts=counts)
