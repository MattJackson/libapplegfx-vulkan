...
void lagfx_protocol_complete_stamp(lagfx_protocol_t *p, uint32_t stamp) {
    ...
    /* Simple design matching an earlier iteration (which got guest reads):
     ...
     * queued stamp and re-writes the cell. This retains an earlier iteration's
...
uint32_t lagfx_protocol_mmio_read(lagfx_protocol_t *p, uint64_t offset) {
     ...
     * Disasm: the internal spec:42-48.
     *
