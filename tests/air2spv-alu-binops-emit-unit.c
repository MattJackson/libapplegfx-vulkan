/*
 * libapplegfx-vulkan — Phase 4 ALU binops emitter unit test
...
static int test_magic_header(void) {
    uint8_t *blob = NULL;
    size_t   sz   = 0u;
    if (lagfx_air2spv_emit_alu_binops_stub(&blob, &sz) != 0 || !blob) {
    ...
    printf("PASS: emit_alu_binops header (size=%zu, version=0x%08x, bound=%u)\n",
           sz, version, bound);
    free(blob);
    return 0;
}
...
static int test_spirv_val(void) {
    ...
    char tmpl[] = "/tmp/lagfx_air2spv_alubinops_XXXXXX.spv";
