/*
 * libapplegfx-vulkan — Phase 4 emit_unop unit test
 * tests/air2spv-unop-emit-unit.c
 *
 * Copyright © 2026 Matthew Jackson
 * SPDX-License-Identifier: MIT
 */

#include "air2spv/emit_unop.h"
...
static int test_magic_header(void) {
    uint8_t *blob = NULL;
    size_t   sz   = 0u;
    if (lagfx_air2spv_emit_unop_stub(&blob, &sz) != 0 || !blob) {
        printf("FAIL: emit failed\n"); return 1;
    }
    if (sz < 20u) { printf("FAIL: %zu\n", sz); free(blob); return 1; }
    uint32_t magic; memcpy(&magic, blob, sizeof(magic));
    if (magic != SPV_MAGIC) { printf("FAIL: magic\n"); free(blob); return 1; }
    if (sz % 4u != 0u) { printf("FAIL: align\n"); free(blob); return 1; }
    printf("PASS: emit_unop module produced (size=%zu)\n", sz);
    free(blob);
    return 0;
}
...
static int test_spirv_val(void) {
    ...
    char tmpl[] = "/tmp/lagfx_air2spv_unop_XXXXXX.spv";
    ...
    printf("PASS: spirv-val accepted the module\n");
    return 0;
}
