/*
 * libapplegfx-vulkan — Phase 4 emit_extinst_glsl unit test
 * tests/air2spv-extinst-glsl-emit-unit.c
 *
 * Copyright © 2026 Matthew Jackson
 * SPDX-License-Identifier: MIT
 */

#include "air2spv/emit_extinst_glsl.h"
...
static int test_magic_header(void) {
    uint8_t *blob = NULL;
    size_t   sz   = 0u;
    if (lagfx_air2spv_emit_extinst_glsl_stub(&blob, &sz) != 0 || !blob) {
    ...
    printf("PASS: emit_extinst_glsl header (size=%zu)\n", sz);
    free(blob);
    return 0;
}
...
static int test_spirv_val(void) {
    ...
    char tmpl[] = "/tmp/lagfx_air2spv_extinst_XXXXXX.spv";
