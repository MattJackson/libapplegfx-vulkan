/*
 * libapplegfx-vulkan — Phase 4 emit_constant_float unit test
...
#include "air2spv/emit_constant_float.h"
...
static int test_magic_header(void) {
    uint8_t *blob = NULL;
    size_t   sz   = 0u;
    if (lagfx_air2spv_emit_constant_float_stub(&blob, &sz) != 0 || !blob) {
        printf("FAIL: emit failed\n");
        return 1;
    }
    if (sz < 20u) {
        printf("FAIL: blob too small (%zu)\n", sz); free(blob); return 1;
    }
    uint32_t magic;
    ...
    if (sz % 4u != 0u) {
        printf("FAIL: size %zu not aligned\n", sz); free(blob); return 1;
    }
    printf("PASS: emit_constant_float header (size=%zu, bound=%u)\n", sz, bound);
    free(blob);
    return 0;
}
...
static int test_spirv_val(void) {
    ...
    char tmpl[] = "/tmp/lagfx_air2spv_cfloat_XXXXXX.spv";
