/*
 * libapplegfx-vulkan — Phase 4 emit_storage_image unit test
...
#include "air2spv/emit_storage_image.h"
...
static int test_magic_header(void) {
    uint8_t *blob = NULL;
    size_t   sz   = 0u;
    if (lagfx_air2spv_emit_storage_image_stub(&blob, &sz) != 0 || !blob) {
    ...
    printf("PASS: emit_storage_image header (size=%zu, bound=%u)\n", sz, bound);
    free(blob);
    return 0;
}
...
static int test_spirv_val(void) {
    ...
    char tmpl[] = "/tmp/lagfx_air2spv_storage_XXXXXX.spv";
