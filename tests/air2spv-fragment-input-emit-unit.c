/*
 * libapplegfx-vulkan — Phase 4 reference emitter unit test: air.fragment_input
 * tests/air2spv-fragment-input-emit-unit.c
 *
 * Copyright © 2026 Matthew Jackson
 * SPDX-License-Identifier: MIT
 *
 * Validates that lagfx_air2spv_emit_fragment_input_stub() produces a
 * Vulkan-compliant SPIR-V module with fragment-stage input + output.
 * At minimum: 5-word header with correct magic. If spirv-val is available
 * on PATH, also runs it on the emitted bytes and asserts exit code 0.
 */

#include "air2spv/emit_fragment_input.h"
...
static int test_magic_header(void) {
    uint8_t *blob = NULL;
    size_t   sz   = 0u;
    if (lagfx_air2spv_emit_fragment_input_stub(&blob, &sz) != 0 || !blob) {
    ...
    printf("PASS: emit_fragment_input header (size=%zu, version=0x%08x, bound=%u)\n",
           sz, version, bound);
    free(blob);
    return 0;
}
...
static int test_spirv_val(void) {
    ...
    printf("PASS: spirv-val accepted emit_fragment_input (%s)\n", spirv_val);
    return 0;
}
