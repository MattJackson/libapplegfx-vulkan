/*
 * libapplegfx-vulkan — Phase 4 emit_vector_shuffle unit test
 * tests/air2spv-vector-shuffle-emit-unit.c
 *
 * Copyright © 2026 Matthew Jackson
 * SPDX-License-Identifier: MIT
 */

#include "air2spv/emit_vector_shuffle.h"
...
static int test_magic_header(void) {
    uint8_t *blob = NULL;
    size_t   sz   = 0u;
    if (lagfx_air2spv_emit_vector_shuffle_stub(&blob, &sz) != 0 || !blob) {
    ...
    printf("PASS: emit_vector_shuffle header (size=%zu)\n", sz);
    free(blob);
    return 0;
}
...
static int test_spirv_val(void) {
    ...
    char tmpl[] = "/tmp/lagfx_air2spv_shuffle_XXXXXX.spv";
